...
const HomePage = () => {
  ...
  return (
    <div className="homepage-shell" style={{ background: 'linear-gradient(180deg, #fff 0%, #fff7f7 45%, #ffffff 100%)', minHeight: '100vh' }}>
      <nav className="navbar navbar-dark sticky-top shadow-sm px-3 px-lg-4" style={{ backgroundColor: '#108132' }}>
        <span className="navbar-brand mb-0 h1 fw-bold fs-3">🍕 Pizzeria</span>
        <div className="d-flex flex-wrap gap-2 justify-content-end">
          <Button variant="light" className="fw-bold" style={{ color: '#07952b' }} onClick={() => navigate('/login')}>Login</Button>
      ...
      <Container className="py-5">
        <Row className="align-items-center g-5 py-4" id="top">
          <Col lg={6} className="homepage-hero">
            <p className="text-uppercase fw-bold mb-3" style={{ color: '#06cb7c', letterSpacing: '0.25rem' }}>Fresh. Fast. Full of flavor.</p>
            <h1 className="homepage-hero-title fw-bold mb-4" style={{ color: '#108132' }}>The modern pizza store for quick orders, premium taste, and easy delivery.</h1>
            ...
            <div className="homepage-hero-actions mb-4">
              <Button size="lg" className="px-4 py-3 fw-bold rounded-pill shadow" style={{ backgroundColor: '#06ba5a', borderColor: '#0cc55c' }} onClick={() => navigate('/login')}>Order Now</Button>
          ...
          {['Pizza', 'Sides', 'Beverages', 'Combos'].map((item, index) => (
            <Col md={6} lg={3} key={item}>
              <Card className="h-100 border-0 shadow-sm rounded-4 text-center p-3">
                <div className="display-6 mb-2" style={{ color: '#18e34e' }}>{['🍕', '🍟', '🥤', '🎁'][index]}</div>
        ...
        <Row className="g-4 my-4 align-items-center" id="about">
          <Col lg={6}>
            <Card className="border-0 shadow-sm rounded-4 p-4">
              <h3 className="fw-bold mb-3" style={{ color: '#00a53a' }}>Why Pizzeria stands out</h3>
        ...
        <Card className="border-0 shadow-sm rounded-4 p-4 my-4" id="contact">
          <Row className="g-4 align-items-center">
            <Col lg={7}>
              <h3 className="fw-bold mb-2" style={{ color: '#00a53a' }}>Contact & support</h3>
